...
AND tests whether the reclaim is a profitable LONG at +0.5%/-0.3% with COST REMOVED vs a random control. Walk-forward."""
...
def run(iv,days,W,Hrec=12,Ht=24,SH=0.0024,TGT=0.005,STOP=0.003,COST=0.0):
    cd=fetch(iv,days)
    ...
    if sht:
        ...
        print(f"  SHALLOW-RECLAIM LONG +0.5%/-0.3%/cost-removed: n={len(sht)} hit+0.5%={hr:.0f}% net={m:+.3f}%/trade | random {cm:+.3f}% | edge {m-cm:+.3f}%")
